...
public class UsbTestLow2 implements Runnable {
	...
	public UsbTestLow2(UsbObject usbObj) {
		currentUsb = usbObj;
	}
	...
	public static void main(String[] args) {
		...
		UsbTestLow2 usbControl = new UsbTestLow2();
		try {
			...
			try {
				...
				int result = usbControl.androidDeviceToAccessoryMode("CsohManufacturer", "CsohModel", "CsohDescription",
						"1.0", "http://www.mycompany.com", "SerialNumber"); // Switch android device to accessory mode
	...
	private int androidDeviceToAccessoryMode(String vendor, String model, String description, String version,
			String url, String serial) throws LibUsbException, GeneralException {
		...
		if (response <= 0) {
			throw new GeneralException(
					"Setup get protocol failed. Error code = " + response);
		}
		...
		response = transferSetupPacket(handle, LibUsb.REQUEST_TYPE_VENDOR,
				(byte) 53);
	...
	private static int transferSetupPacket(DeviceHandle handle,
			byte requestType, byte request) throws LibUsbException {
		int response = 0;
		...
		response = LibUsb.controlTransfer(handle, requestType, request, wValue,
				wIndex, data, timeout);
	...
	private static int transferAccessoryDataPacket(DeviceHandle handle,
			String param, short index) {
		int response;
		...
		final short wValue = 0;
		final long timeout = 0;
		response = LibUsb.controlTransfer(handle, LibUsb.REQUEST_TYPE_VENDOR,
				bRequest, wValue, index, data, timeout);
		if (response < 0)
			throw new LibUsbException("Unable to control transfer.", response);
		return response;
	}

	private static int transferBulkData(DeviceHandle handle, byte endpoint,
		...
		int result = LibUsb.bulkTransfer(handle, endpoint, buffer, transfered,
				timeout);
