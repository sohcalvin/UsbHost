...
public class AdbDevice {
     ...
     *            THe out endpoint address.
     */
    public AdbDevice(UsbInterface iface, byte inEndpoint, byte outEndpoint) {
	if (iface == null)
	...
	UsbEndpoint outEndpoint = this.iface.getUsbEndpoint(this.outEndpoint);
	UsbPipe outPipe = outEndpoint.getUsbPipe();
	MessageHeader header = message.getHeader();
	outPipe.open();
	try {
	    int sent = outPipe.syncSubmit(header.getBytes());
	    if (sent != MessageHeader.SIZE)throw new InvalidMessageException("Invalid ADB message header size sent: " + sent);
	    sent = outPipe.syncSubmit(message.getData());
	    if (sent != header.getDataLength())	throw new InvalidMessageException("Data size mismatch in sent ADB message. Should be "	+ header.getDataLength() + " but is " + sent);
	} finally {
	    outPipe.close();
	}
    }
	...
	UsbEndpoint inEndpoint = this.iface.getUsbEndpoint(this.inEndpoint);
