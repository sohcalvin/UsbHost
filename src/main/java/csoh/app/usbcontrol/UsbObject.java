	...
	public UsbObject(short vendorId, short productId)
		...
		if (device == null)
			throw new DeviceNotFoundException(
					"Unable to find device for vendorId=" + vendorId
	...
	public static ArrayList<HashMap<DESCRIPTOR_FIELD, Object>> getConnectedDeviceProperties(
		...
		try {
			for (Device device : list) {
				...
				String vendorName = ResourceManager.getInstance()
						.getVendorName(vendorId);
				String productName = ResourceManager.getInstance()
				...
				if (productName == null) {
					...
					if (len > lenFromRight)
						hexProductId = hexProductId.substring(0, len
								- lenFromRight);
				...
				prop.put(DESCRIPTOR_FIELD.MANUFACTURER,
						descriptor.iManufacturer());
				prop.put(DESCRIPTOR_FIELD.PRODUCT, descriptor.iProduct());
				prop.put(DESCRIPTOR_FIELD.SERIAL_NUMBER,
